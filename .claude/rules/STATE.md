...
1. Grid rendering: draw sacks/tabs as actual grids with items at
   their cells (list view today), then drag-and-drop movement. Item
   icons are within reach now — `GameData::resource` can extract the
   `.tex` bitmaps and the DDS payload is standard.
